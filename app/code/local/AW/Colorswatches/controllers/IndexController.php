...
class AW_Colorswatches_IndexController extends Mage_Core_Controller_Front_Action
{
    ...
    public function indexAction()
    {
        ...
        if (!is_null($product->getData('image')) && $this->_config('image')
            ...
            $image = $this->loadZoomImages($product->getId());// $imageBlock->toHtml();
        }
        ...
        $dataArray = array(
            ...
            'price'            => $price,
            'customZoomEnabled'=>Mage::getStoreConfig('shopgo_zoom/general/enabled')
        );
    ...
    // Custom Function to get  the images to Shopgo Zoom V3
    public function loadZoomImages($productId){
        if(Mage::getStoreConfig('shopgo_swinzoom/general/enabled')){
            $imageBlock = $this->getLayout()->createBlock('catalog/product_view_media');
            $imageBlock->setTemplate('shopgo/swinzoom/media.phtml');
