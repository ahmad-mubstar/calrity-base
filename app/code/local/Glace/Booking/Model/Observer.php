 ...
 *
*/
class Glace_Booking_Model_Observer extends Mage_Core_Model_Observer{
	...
	public function validateBookingInOrder(Varien_Event_Observer $observer){
		...
		if(!$order->getId())
		{
			if(!$this->validateBookOptions($order))
				Mage::throwException(Mage::helper('booking')->__('Can`t be booked'));
			else return $this;
		}
		return $this;
	}
	...
	public function cleanCanceledBooks(Varien_Event_Observer $observer){
		...
		{
			$book_item = Mage::getModel('booking/book_item')->load($item->getId(), 'order_item_id');
			if($book_item->getId())
				$book_item->delete();
		}
		return true;
	...
	public function cleanCreditMemoBooks(Varien_Event_Observer $observer){
		...
		foreach ($creditmemo->getAllItems() as $item) {
			...
			{
				$book_item = Mage::getModel('booking/book_item')->load($item->getOrderItem()->getId(), 'order_item_id');
				if($book_item->getId())
					$book_item->delete();
			}
		}
	...
	public function validateBookOptions($order){
		...
		$helper = Mage::helper('booking/availible');
		$qtys = array();
				...
				foreach ($options as $option){
					if($option['option_type'] == 'multidate_type'){
						...
						if(!(count($values) > 1 && is_numeric($values[0])))
							Mage::throwException(Mage::helper('booking')->__('Booking data is in wrong format.'));
		...
		foreach ($qtys as $product_id => $values){
			foreach ($values as $day => $qty){
				if($helper->isAvailible($product_id, $day, $qty) === false){
					...
					Mage::throwException(Mage::helper('booking')->__($product->getName(). ' can`t be book in that quantity.'));
				}	
			}	
		}
	...
	public function saveBookOptions(Varien_Event_Observer $observer){
		...
		$dateTo   = '';
		try{
			$book = Mage::getModel('booking/book');
	...
	public function saveBookItem($book_id, $product_id, $item_id, $dateFrom, $dateTo, $qty){
		$book_item = Mage::getModel('booking/book_item');
	...
	public function checkCartAdd($observer) {
		...
		{
	        $max = Mage::app()->getRequest()->getParam('cmsideasday_max_qty');
	        $qty = Mage::app()->getRequest()->getParam('qty');
	        
	        if($max < $qty){
	        	Mage::throwException(Mage::helper('booking')->__('Such quantity to book is not availible.'));
	        }
		}
        return $this;
	}
