...
*
*/
class Glace_Booking_Model_Product_Attribute_Backend_Customsession
extends Mage_Eav_Model_Entity_Attribute_Backend_Abstract{
	...
	{
		return Mage::getResourceSingleton('booking/product_attribute_backend_customsession');
	}
	...
	public function afterSave($object)
	{
		...
		foreach ($customSessions as $session){
			$session_object = Mage::getModel('booking/book_session');
			...
			if(isset($session['session_times'])){
				foreach ($session['session_times'] as $time){
					$time_object = Mage::getModel('booking/book_session_time');
	...
	public function afterLoad($object){
		...
		$data = array();
		$collection = Mage::getModel('booking/book_session')->getCollection()->addFilter('entity_id', $object->getId());
		
		foreach ($collection as $item){
			$pom = array();
			foreach ($item->getData() as $key => $value){
				if($key == 'session_id'){
					$time_collection = Mage::getModel('booking/book_session_time')->getCollection()->addFilter('session_id', $value);
					$times = array();
