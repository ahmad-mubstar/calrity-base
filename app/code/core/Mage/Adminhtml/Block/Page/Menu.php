...
//        }
    ...
    protected function _buildMenuArray(Varien_Simplexml_Element $parent=null, $path='', $level=0)
    {
        ...
        foreach ($parent->children() as $childName => $child) {
            ...
            $menuArr['name'] = $childName;
            // Mage::log($menuArr, Zend_Log::DEBUG, 'test.log');
            // $menuArr['module'] = $child->attributes()['module'];
    ...
    public function getVMenuLevel($menu, $level = 0)
    {
        $html = '<ul class="mcd-menu" ' . (!$level ? 'id="nav1"' : '') . '>' . PHP_EOL;
        foreach ($menu as $key => $item) {

                //print_r($item);
//                if ($level == 0) {
//                    Mage::log($key . ' ' . $item['label'], Zend_Log::DEBUG, 'test.log');
//                }
                $class = 'fa fa-home';

                switch($key) {
                    case 'dashboard': $class = 'fa fa-dashboard'; break;
                    case 'sales': $class = 'fa fa-line-chart'; break;
                    case 'catalog': $class = 'fa fa-folder'; break;
                    case 'customer': $class = 'fa fa-users'; break;
                    case 'marketing': $class = 'fa fa-gift'; break;
                    case 'promo': $class = 'fa fa-gift'; break;
                    case 'newsletter': $class = 'fa fa-newspaper-o'; break;
                    case 'Manufacturer': $class = 'fa fa-industry'; break;
                    case 'cms': $class = 'fa fa-columns'; break;
                    case 'Content': $class = 'fa fa-columns'; break;
                    case 'Blog': $class = 'fa fa-comment'; break;
                    case 'Mega Menu': $class = 'fa fa-bars'; break;
                    case 'MANAdev': $class = 'fa fa-mouse-pointer'; break;
                    case 'report': $class = 'fa fa-file-text'; break;
                    case 'Shopgo': $class = 'fa fa-users'; break;
                    case 'system': $class = 'fa fa-cogs'; break;
                }
            ...
            $html .= '<li ' . (!empty($item['children']) ? 'onmouseover="Element.addClassName(this,\'over\')" '
                ...
                . ($level == 0 ? '<i class="'. $class .'"></i>' : '')

                . '<span>'
                . $this->escapeHtml($item['label']) . '</span>';

                if (!empty($item['children'])) {
                    $html .= '<i style="position: absolute;right:5px;top:10px;font-size:14px;" class="fa  fa-angle-right"></i>' . PHP_EOL;
                }

                $html .= '</a>' . PHP_EOL;

            if (!empty($item['children'])) {
                $html .= $this->getVMenuLevel($item['children'], $level + 1);
            }
