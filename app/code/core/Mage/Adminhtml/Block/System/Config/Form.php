...
class Mage_Adminhtml_Block_System_Config_Form extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    public function initForm()
    {
        ...
        foreach ($sections as $section) {
            ...
            if($sec == 'bannerslider') {
                if (! Mage::getSingleton('admin/session')->isAllowed('admin/shopgo/' . $sec . '/config')) continue;
            }
            if($sec == 'switcher') {
                if (! Mage::getSingleton('admin/session')->isAllowed('admin/system/config/' . $sec)) continue;
            }
            ...
            foreach ($section->groups as $groups){
                ...
                foreach ($groups as $group){
                    ...
                    // echo $group->getName() . ' --> ' . $group->label  .'<br/>';
                    // isalem : 8/11/2017

                    $value = $section->getName() . '/' . $section->getName() . '_' . $group->getName();
                    if (in_array($value, ['currencymanager/currencymanager_info','web/web_secure','web/web_unsecure','design/design_package','design/design_theme'])) {
                        if (!Mage::getSingleton('admin/session')->isAllowed('admin/system/config/' . $value)) continue;
                    }

                    if ($section->getName() == 'carriers') {
                        if (!Mage::getSingleton('admin/session')->isAllowed('admin/system/config/carriers/' . $group->getName())) continue;
                    }

                    if ($section->getName() == 'payment') {
                        if (!Mage::getSingleton('admin/session')->isAllowed('admin/system/config/payment/' . $group->getName())) continue;
                    }
