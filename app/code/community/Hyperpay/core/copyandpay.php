<?php
	...
	if(!function_exists('getPostParameter')){
		function getPostParameter($dataCust,$dataTransaction) 
		{
            ...
            $data = "authentication.userId=".$dataTransaction['userId'].
                "&authentication.password=".$dataTransaction['password'].
                "&authentication.entityId=".$dataTransaction['entityId'].
	...
	if(!function_exists('getToken')){
		function getToken($postData,$url,$server)
		{
            $test = true;
            if ($server=='live')
            {
	...
	if(!function_exists('getStatusUrl')){
		function getStatusUrl($server, $token,$order){
            ...
            $userId = Mage::getStoreConfig('payment/hyperpay/userId', $order->getStoreId());
            $entityId = Mage::getStoreConfig('payment/' . $code . '/entityId', $order->getStoreId());
            $password = Mage::getStoreConfig('payment/hyperpay/password', $order->getStoreId());
			if ($server=="live")
	...
	if(!function_exists('checkStatusPayment')){
		function checkStatusPayment($server,$url)
		{
			$test = true;
			if ($server=='live')
			{
