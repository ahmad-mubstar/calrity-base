...
class Magestore_Megamenu_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function setGroupedTemplateDefaults()
    {
        $templateFolder = 'group_category_listing';
        $filename = 'default.phtml';
        $block = Mage::app()->getLayout()->createBlock('megamenu/item');
        $data = $block->setTemplate('megamenu/templates/'.$templateFolder.'/'.$filename)->toHtml();
        $data = $block->getData();

        $menuCollection = Mage::getModel('megamenu/megamenu')->getCollection()
                          ->addFieldToFilter('menu_type', 5);

        try {
            foreach ($menuCollection as $menu) {
                $menu->setBackgroundColor(str_replace('#', '', $data['general_style']['background_color']));
                $menu->setBorderColor(str_replace('#', '', $data['general_style']['border_color']));
                $menu->setBorderSize($data['general_style']['border_size']);
                $menu->setTitleColor(str_replace('#', '', $data['title_style']['title_color']));
                $menu->setTitleBackgroundColor(str_replace('#', '', $data['title_style']['title_background_color']));
                $menu->setTitleFont($data['title_style']['title_font']);
                $menu->setTitleFontSize($data['title_style']['title_font_size']);
                $menu->setSubtitleColor(str_replace('#', '', $data['subtitle_style']['subtitle_color']));
                $menu->setSubtitleFont($data['subtitle_style']['subtitle_font']);
                $menu->setSubtitleFontSize($data['subtitle_style']['subtitle_font_size']);
                $menu->setLinkColor(str_replace('#', '', $data['link_style']['link_color']));
                $menu->setHoverColor(str_replace('#', '', $data['link_style']['hover_color']));
                $menu->setLinkFont($data['link_style']['link_font']);
                $menu->setLinkFontSize($data['link_style']['link_font_size']);
                $menu->setTextColor(str_replace('#', '', $data['text_style']['text_color']));
                $menu->setTextFont($data['text_style']['text_font']);
                $menu->setTextFontSize($data['text_style']['text_font_size']);
                $menu->setColum($data['content_general']['colum']);
                $menu->setSizeColum($data['content_general']['size_colum']);
    ...
    public function parseAnchorTextLink($link)
    {
