...
class Magestore_Megamenu_Block_Navigationtop extends Mage_Catalog_Block_Navigation
{
    ...
    public function getMenuEffect()
    {
        ...
        $currentTheme = Mage::getDesign()->getTheme('template');

        $animationThemes = array(
            'tshirt', 'tshirt_ar',
            'minimal', 'minimal_ar',
            'gifts', 'gifts_ar',
            'chocolate', 'chocolate_ar'
        );

        switch (true) {
            case in_array($currentTheme, $animationThemes):
                $effect = 2;
                break;
        }
    ...
    public function getMenuDirection()
    {
        $store = $this->getStore();
        if(!$store) {
            $store = Mage::app()->getStore()->getId();
        ...
        $leftDirectionThemes = array(
            'compact', 'compact_ar', 'printables'
        );

        $isLeftTheme = false;

        // TODO: Not the best implementation, but just a temporary workaround.
        foreach ($leftDirectionThemes as $_theme) {
            if (strpos($currentTheme, $_theme) !== false) {
                $isLeftTheme = true;
                break;
            }
        }
...
//      }
    ...
    public function getMenuWidth() {
        ...
        switch (true) {
            case strpos($currentTheme, 'compact') !== false:
                $menuWidth = '100%';
                break;
        }
    ...
    public function getMenuHeight(){
        ...
        switch (true) {
            case strpos($currentTheme, 'compact') !== false:
                $menuHeight = 'auto';
                break;
            case strpos($currentTheme, 'printables') !== false:
                $menuHeight = 30;
                break;
        }
    ...
    {
        return Mage::helper('megamenu')->parseAnchorTextLink($link);
    }
}
