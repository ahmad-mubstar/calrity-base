...
class Shopgo_Social_Model_Twitter_Oauth_Client
{
    const REDIRECT_URI_ROUTE = 'social/twitter/connect';
    const REQUEST_TOKEN_URI_ROUTE = 'social/twitter/request';

    const OAUTH_URI = 'https://api.twitter.com/oauth';
    const OAUTH2_SERVICE_URI = 'https://api.twitter.com/1.1';    

    const XML_PATH_ENABLED = 'social/twitter/enabled';
    const XML_PATH_CLIENT_ID = 'social/twitter/client_id';
    const XML_PATH_CLIENT_SECRET = 'social/twitter/client_secret';
    ...
    protected function _httpRequest($url, $method = 'GET', $params = array())
    {
        ...
        if($response->isError()) {
            ...
            if(($status == 400 || $status == 401 || $status == 429)) {
                ...
                throw new Shopgo_Social_Model_Twitter_Oauth_Exception($message);
            } else {
                $message = sprintf(
                    Mage::helper('shopgo_social')
