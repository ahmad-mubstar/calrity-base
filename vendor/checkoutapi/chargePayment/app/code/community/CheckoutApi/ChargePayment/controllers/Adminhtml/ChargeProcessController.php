        ...
        if($hasBeenCaptured) {
            ...
            if ($_payment->getMethodInstance() instanceof CheckoutApi_ChargePayment_Model_Method_Abstract) {
                ...
                if(!$transactionCapture->getOrderId()) {
                    ...
                    $_payment->setTransactionId($_captureCharge->getId());
                    $_payment->setParentTransactionId($_captureCharge->getOriginalId());
                  ...
                  with Transaction ID ' . $_captureObj->getId () );
                    ...
                    $_order->save ();
                    Mage::getSingleton ( 'adminhtml/session' )->addSuccess ( 'Payment Sucessfully captured
                  with Transaction ID ' . $_captureObj->getId () );
                }else {
    ...
    public  function  VoidAction()
    {
        ...
        if($_refundCharge->isValid() && $_refundCharge->getStatus() == 'Voided' &&
            ...
            if ($_payment->getMethodInstance() instanceof CheckoutApi_ChargePayment_Model_Method_Creditcard) {
                ...
                $_order->getPayment()
                    ->setTransactionId($_refundCharge->getId())
                    ->setParentTransactionId($_refundCharge->getOriginalId())
                    ->void( new Varien_Object());
            }
            $_order->registerCancellation('Transaction has been voided')
                ->save();
                ...
                $_payment->addTransaction(Mage_Sales_Model_Order_Payment_Transaction::TYPE_VOID, null, false , 'Transaction has been voided');
                $_payment->void(
                    $_voidObj
                );
                $_payment->unsLastTransId();
                Mage::getSingleton ( 'adminhtml/session' )->addSuccess ( 'Transaction ID ' . $_refundCharge->getOriginalId().' has been voided successfully' );
